...
export default function ExpiryLowStock() {
  ...
  const enriched = products.map((p) => {
    ...
    const expiryDays = Math.floor(diff);
    const isExpiring = expiryDays <= 30 && expiryDays >= 0;
    const isExpired = expiryDays < 0;
    const isLowShelf = p.shelfStock < 10;
    const isLowRetail = p.retailStock < 30;
    return { ...p, expiryDays, isExpiring, isExpired, isLowShelf, isLowRetail };
  });
  ...
  const critShelf = enriched.filter((p) => p.shelfStock < 10).length;
  const critRetail = enriched.filter((p) => p.retailStock < 30).length;
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-red-50 rounded-xl p-4 border border-red-200">
          ...
          <p className="text-red-600 font-bold text-2xl">{critShelf}</p>
        ...
        <div className="bg-orange-50 rounded-xl p-4 border border-orange-200">
          ...
          <p className="text-orange-600 font-bold text-2xl">{critRetail}</p>
              ...
              {filtered.map((p) => (
                <tr key={p.id} className={`hover:bg-slate-50/50 ${p.isExpired ? "bg-red-50/30" : ""}`}>
                  ...
                  <td className={`px-4 py-3 text-right font-bold text-xs ${p.isExpired ? "text-red-600" : p.isExpiring ? "text-amber-600" : "text-slate-500"}`}>
                    {p.isExpired ? `Expired ${Math.abs(p.expiryDays)}d ago` : `${p.expiryDays}d`}
                  </td>
