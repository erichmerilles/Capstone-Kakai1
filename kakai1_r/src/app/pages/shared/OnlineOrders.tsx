...
export default function OnlineOrders() {
  ...
  const [newOrder, setNewOrder] = useState({ customer: "", contact: "", address: "", source: "Facebook", payment: "GCash", note: "" });

  const fetchOrders = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_URL}/orders/get_online_orders.php`, { credentials: "include" });
  ...
  const updateStatus = async (id: number, status: string) => {
    try {
      const response = await fetch(`${API_URL}/orders/update_order_status.php`, {
        method: "POST",
  ...
  const createOrder = async () => {
    if (!newOrder.customer) return alert("Customer name is required.");

    try {
      const response = await fetch(`${API_URL}/orders/create_online_order.php`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {["Pending", "Processing", "Ready", "Delivered"].map((s) => (
          <div key={s} className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
            <p className="text-slate-400 text-xs">{s}</p>
            <p className="text-slate-800 font-bold text-xl">{isLoading ? "-" : orders.filter((o) => o.status === s).length}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-1 bg-slate-100 rounded-lg p-0.5 overflow-x-auto">
          {["All", "Pending", "Processing", "Ready", "Delivered", "Cancelled"].map((s) => (
            <button key={s} onClick={() => setFilter(s)} className={`px-2.5 py-1 rounded-md text-xs font-medium whitespace-nowrap transition-colors ${filter === s ? "bg-white shadow text-slate-700" : "text-slate-400 hover:text-slate-600"}`}>{s}</button>
          ))}
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading ? (
                <tr><td colSpan={8} className="py-8 text-center text-slate-400">Loading orders...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={8} className="py-8 text-center text-slate-400">No orders found.</td></tr>
              ) : filtered.map((o) => (
                <tr key={o.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-slate-700">₱{Number(o.total).toLocaleString()}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-1 rounded-full border font-medium ${statusColor[o.status] || "bg-slate-50 text-slate-500"}`}>{o.status}</span>
                  </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-slate-800 font-semibold">Order #{selectedOrder.id}</h2>
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><p className="text-slate-400 text-xs">Contact</p><p className="text-slate-700">{selectedOrder.contact || "N/A"}</p></div>
                ...
                <div className="col-span-2"><p className="text-slate-400 text-xs">Address</p><p className="text-slate-700">{selectedOrder.address || "Store Pickup"}</p></div>
              </div>

              <div>
                <p className="text-slate-500 text-xs font-medium mb-2">Order Notes & Items</p>
                <div className="bg-slate-50 rounded-lg p-3 text-sm text-slate-700 whitespace-pre-wrap border border-slate-100">
                  {selectedOrder.note || "No specific items noted."}

                  {/* Future proofing if you ever link real items */}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-slate-500 text-xs mb-1 block">Contact Number</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-500 text-xs mb-1 block">Order Source</label>
              ...
              <div>
                <label className="text-slate-500 text-xs mb-1 block">Order Note / Items Requested</label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={createOrder} className="flex-1 bg-orange-500 hover:bg-orange-600 text-white rounded-lg py-2.5 text-sm font-medium transition-colors">Save Order</button>
              </div>
            </div>
          </div>
        </div>
